...
if(nameKey.value.length==0){
	...
	GCD.innerHTML="GCD of key and 26 is equal to 1 &#x2715;"	
}
...
nameKey.addEventListener("input",()=>{
	...
	if(nameKey.value){
		if(gcd!=1||nameKey.value.length==0){
			GCD.style.color="#e74c3c"		
			GCD.innerHTML="GCD of key and 26 is equal to 1 &#x2715;"
		}
		if(gcd==1&&nameKey.value.length!=0){
			GCD.style.color="#2ecc71"		
			if(GCD.innerText=="GCD of key and 26 is equal to 1 ✕"){
				GCD.innerHTML="GCD of key and 26 is equal to 1 &#10003;";
			}
			...
			for(let i=0;i<textAreaEncrypt.value.length;i++){
				if(textAreaEncrypt.value[i]==" "){
					...
					textAreaDecrypt.value=textAreaDecrypt.value+String.fromCharCode(((textAreaEncrypt.value[i].charCodeAt(0)-65)*key%26+65));	
				}
				else if(textAreaEncrypt.value[i]==textAreaEncrypt.value[i].toLowerCase()){
					textAreaDecrypt.value=textAreaDecrypt.value+String.fromCharCode(((textAreaEncrypt.value[i].charCodeAt(0)-97)*key%26+97));	
				}	
			}
		}
	}else{
  ...
  if(key>25){
	...
	for(let i=0;i<textAreaDecrypt.value.length;i++){
		if(textAreaDecrypt.value[i]==" "){
			...
			let a=(textAreaDecrypt.value[i].charCodeAt(0)-65)*t1;
			if(a<0){
				a=a+26;
				let m=(a%26)+65;
			...
			let a=(textAreaDecrypt.value[i].charCodeAt(0)-97)*t1;
			if(a<0){
				a=a+26;
				let m=(a%26)+97;
				textAreaEncrypt.value=textAreaEncrypt.value+String.fromCharCode(m)				
