...
nameKey.addEventListener("input",()=>{
    ...
    keyList=[]
    let keyListArr=nameKey.value.split('');
    for(let i=0;i<keyListArr.length;i++){
      keyList.push(keyListArr[i]);
    }
    ...
    for(let i=0;i<textAreaDecrypt.value.length;i++){
      ...
      if(textAreaDecrypt.value[i]==" "){
      ...
      else if((ignore.includes(keyList[start])&&typeof(keyList[start])!='number')||keyList[start]==" "){
        textAreaEncrypt.value=textAreaEncrypt.value+textAreaDecrypt.value[i];
        start++;
      }
      else if(textAreaDecrypt.value[i]==textAreaDecrypt.value[i].toUpperCase()){
        if(typeof(keyList[start])=='string'){
          if(keyList[start]==keyList[start].toLowerCase()){
            keyList[start]=keyList[start].charCodeAt(0)-97;
          }else if(keyList[start]==keyList[start].toUpperCase()){
            keyList[start]=keyList[start].charCodeAt(0)-65;
          }
        }
        let a=textAreaDecrypt.value[i].charCodeAt(0)-keyList[start]-65;
        if(a<0){
          a=a+26;
          let m=(a%26)+65;
        ...
        let a=textAreaDecrypt.value[i].charCodeAt(0)-keyList[start]-97;
        if(a<0){
          a=a+26;
          let m=(a%26)+97;
          textAreaEncrypt.value=textAreaEncrypt.value+String.fromCharCode(m)	
      ...
      if(keyList.length>=i){
        if(textAreaEncrypt.value[i]==" "){
          ...
          keyList.push(textAreaEncrypt.value[i].charCodeAt(0)-65);			
        }
        else if(textAreaEncrypt.value[i]==textAreaEncrypt.value[i].toLowerCase()){
          keyList.push(textAreaEncrypt.value[i].charCodeAt(0)-97);
        }	
      }
    }
    ...
    if(nameKey.value==""){
      ...
      nameKey.focus();
		...
		textAreaDecrypt.value=textAreaDecrypt.value.slice(0,-(lenDecrypt-textAreaDecrypt.value.length));	
	}
...
textAreaEncrypt.addEventListener("input",()=>{
  ...
  keyList=[]
	...
	const ignore="1234567890~`!@#$%^&*()_-+={[}]|:;\"'\\<>,./?"
  for(let i=0;i<textAreaEncrypt.value.length-nameKey.value.length;i++){
		if(textAreaEncrypt.value[i]==" "){
		}
		else if(ignore.includes(textAreaEncrypt.value[i])){
		}
  ...
  let start=0;
	for(let i=0;i<textAreaEncrypt.value.length;i++){
      ...
      start++;
			...
			textAreaDecrypt.value=textAreaDecrypt.value+String.fromCharCode((textAreaEncrypt.value[i].charCodeAt(0)+keyList[start]-65)%26+65)
      start++;
		}
		else if(textAreaEncrypt.value[i]==textAreaEncrypt.value[i].toLowerCase()){
			if(typeof(keyList[start])=='string'){
			...
			textAreaDecrypt.value=textAreaDecrypt.value+String.fromCharCode((textAreaEncrypt.value[i].charCodeAt(0)+keyList[start]-97)%26+97)
      start++;
		}	
	}
