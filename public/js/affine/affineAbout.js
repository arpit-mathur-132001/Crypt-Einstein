...
function myFunction1(y) {
  ...
  if(y.matches) { // If media query matches
    ...
    const additiveAbout=document.getElementById("additiveAbout");
		if(additiveAbout){
			additiveAbout.addEventListener("click",()=>{
				ulvnav(0);
				setTimeout(() => {
					window.location.href="../additive/additiveAbout";
				}, 500);
			})
		}
		
		const multiplicativeAbout=document.getElementById("multiplicativeAbout");
		if(multiplicativeAbout){
			multiplicativeAbout.addEventListener("click",()=>{
				ulvnav(1);
				setTimeout(() => {
					window.location.href="../multiplicative/multiplicativeAbout";
				}, 500);
			})
		}
		
		const autokeyAbout=document.getElementById("autokeyAbout");
		if(autokeyAbout){
			autokeyAbout.addEventListener("click",()=>{
				ulvnav(2);
				setTimeout(() => {
					window.location.href="../autokey/autokeyAbout";
				}, 500);
			})
		}
		
		const rsaAbout=document.getElementById("rsaAbout");
		if(rsaAbout){
			rsaAbout.addEventListener("click",()=>{
				ulvnav(3);
				setTimeout(() => {
					window.location.href="../rsa/rsaAbout";
				}, 500);
			})
		}
		
		const affineAbout=document.getElementById("affineAbout");
		if(affineAbout){
			affineAbout.addEventListener("click",()=>{
				ulvnav(4);
				setTimeout(() => {
					window.location.href="../affine/affineAbout";
				}, 500);
			})
		}
